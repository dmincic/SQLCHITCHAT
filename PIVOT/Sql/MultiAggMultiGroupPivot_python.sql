...
EXEC sys.sp_execute_external_script
        ...
        ,@script   = N'
import numpy as np
dfpivot_out = df.pivot_table(index = ["Shipcountry","Shipregion"], \
                              columns =["OrderYear"], \
                              values = ["Freight","OrderValue"], \
                              aggfunc={"Freight":sum,"OrderValue":np.average}).reset_index(level=["Shipcountry","Shipregion"])
print(dfpivot_out)
' 
   ...
   WITH RESULT SETS  ((Shipcountry NVARCHAR(15),Shipregion NVARCHAR(15),[2018] FLOAT, [2019] FLOAT,[2020] FLOAT
